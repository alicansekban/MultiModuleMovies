...
import com.alican.data.data.response.BasePagingResponse
...
class ApiService @Inject constructor(
    ...
    suspend fun getUpComingMovies(page: Int) : ResultWrapper<BasePagingResponse<MovieResponse>> =
        safeApiCall(Dispatchers.IO) {
            client.get("${Constants.UP_COMING_ENDPOINT}$page").body()
        }
    suspend fun getPopularMovies(page: Int) : ResultWrapper<BasePagingResponse<MovieResponse>> =
        safeApiCall(Dispatchers.IO) {
            client.get("${Constants.POPULAR_ENDPOINT}$page").body()
        }
    suspend fun getTopRatedMovies(page: Int) : ResultWrapper<BasePagingResponse<MovieResponse>> =
        safeApiCall(Dispatchers.IO) {
            client.get("${Constants.TOP_RATED_ENDPOINT}$page").body()
        }
    suspend fun getNowPlayingMovies(page: Int) : ResultWrapper<BasePagingResponse<MovieResponse>> =
        safeApiCall(Dispatchers.IO) {
            client.get("${Constants.NOW_PLAYING_ENDPOINT}$page").body()
        }
    suspend fun searchMovie(page: Int, query: String) : ResultWrapper<BasePagingResponse<MovieResponse>> =
        safeApiCall(Dispatchers.IO) {
